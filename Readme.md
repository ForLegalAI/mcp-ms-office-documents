...
A soft break never swallows what follows it: a list, table, heading or page break on the next line is still that block. A `<br>` that a model wrote *before* a list (`Intro:<br>1. A<br>2. B`) is promoted to a real line break so the list renders as one — which is why a line-leading number that is really text (`1. máje 5`) needs the documented `1\.` escape either way.
